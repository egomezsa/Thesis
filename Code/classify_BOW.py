...
def extract_features(sampled_dict, training_set):
	...
	feature_vector = np.zeros((len(EMOTION_LIST)*training_set, VECTOR_SIZE))
	test_vector = np.zeros((len(EMOTION_LIST)*TEST_SAMPLES, VECTOR_SIZE))
	class_vector = []
	test_class = []
	...
	for em in EMOTION_LIST:
		...
		vectors = np.zeros((training_set+TEST_SAMPLES,VECTOR_SIZE))
		count = 0
