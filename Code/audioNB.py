...
def test_classifier(clf, test_set):
	res =  MT.post_process_audio(clf.predict(test_set))

	results = map(lambda x:EMOTION_LIST[x.index(max(x))], res)
...
def simple_train(training_set, tst):
	...
	au_features = MT.extract_features_Audio(sample_dict, training_set, TEST_SAMPLES, tst, normalize = 'M' in tst[0])

	feature_vector = au_features[i_dict['feature_train']]
	class_vector = au_features[i_dict['class_train']]
...
if len(sys.argv) < 2:
	inputstr = './run/Audio.p'
else:
	tst = sys.argv[1]
	inputstr = './run/Audio_'  + sys.argv[1] + '.p'
